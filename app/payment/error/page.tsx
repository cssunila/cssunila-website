...
import { XCircle, RefreshCw, ArrowRight, Loader2, ShieldAlert } from "lucide-react";
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        setResult({
          state: transactionStatus === "expire" ? "expired" : "error",
      ...
      <div className="w-full max-w-lg">
        {result.state === "loading" && (
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-muted-foreground text-sm">Memuat informasi pembayaran…</p>
          </div>
        )}
        ...
        {result.state === "invalid" && (
          <div className="glass rounded-3xl border border-white/10 p-10 text-center shadow-2xl">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Halaman ini hanya dapat diakses setelah terjadi kegagalan proses pembayaran Midtrans.
        ...
        {(result.state === "error" || result.state === "expired") && (
          <div className="glass rounded-3xl border border-red-500/20 p-10 text-center shadow-2xl">
            {/* Icon */}
            <div className="mx-auto mb-6 flex size-24 items-center justify-center rounded-full bg-red-500/15 shadow-2xl shadow-red-500/20">
            ...
            <p className="mt-3 text-muted-foreground text-sm leading-relaxed">
              {result.state === "expired"
                ? "Sesi pembayaran telah habis masa berlakunya. Kamu perlu membuat sesi pembayaran baru untuk melanjutkan pendaftaran."
                : "Terjadi kesalahan saat memproses pembayaranmu. Ini mungkin disebabkan oleh saldo tidak mencukupi, transaksi ditolak oleh bank, atau koneksi terputus."}
            </p>
            ...
            <div className="mt-5 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold bg-red-500/15 text-red-300">
              <XCircle size={12} />
              {result.state === "expired" ? "Sesi Kedaluwarsa" : "Pembayaran Ditolak"}
            </div>

            {/* Tips */}
            <div className="mt-6 rounded-2xl bg-red-500/5 border border-red-500/15 p-4 text-left">
              <p className="text-xs font-semibold text-red-400 mb-2">Kemungkinan penyebab:</p>
              <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                {result.state === "expired" ? (
                  <>
                    <li>Waktu sesi pembayaran (biasanya 24 jam) telah habis</li>
                    <li>Sesi dibuka terlalu lama tanpa menyelesaikan pembayaran</li>
                  </>
                ) : (
                  <>
                    <li>Saldo atau limit kartu tidak mencukupi</li>
                    <li>Transaksi ditolak oleh bank penerbit</li>
                    <li>Koneksi internet terputus saat pembayaran</li>
                    <li>Nomor kartu atau data pembayaran tidak valid</li>
                  </>
                )}
              </ul>
            </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              ...
              >
                <RefreshCw size={14} /> Coba Bayar Ulang
              </Link>
              <Link
                href="/"
